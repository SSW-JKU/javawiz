...
export default defineComponent({
        ...
        reader.onload = () => {
          try {
            if (fileExtension === 'java') {
              vm.generalStore.fileManager.addFile(file.name, reader.result!.toString(), 'java', true)
            } else {
              // this should already be prevented by the "accept"-attribute of the file input
              this.generalStore.notifications.show({ kind: 'InvalidFileType' })
            }
          } catch (_) {
            this.generalStore.notifications.show({ kind: 'FileReadError' })
          }
        }
  ...
  methods: {
    ...
    saveFiles: function () {
      ...
      const files = vm.generalStore.fileManager.fileContents
