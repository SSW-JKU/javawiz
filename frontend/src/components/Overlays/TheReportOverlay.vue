  ...
  <Overlay :show="overlayStore.showReport" title="Report Issue / Suggestion" @close-dialog="hide">
    <form v-if="!submitSuccess" class="report-form" @submit.prevent="submitReport">
      ...
      <div class="form-group markdown-group">
        <label for="reportMarkdown">Details (Markdown supported): <span class="required">*</span> ({{ reportMarkdown.length }} / {{ MAX_DISCORD_LENGTH }})</label>
        <div class="markdown-editor">
          <textarea
            ...
            rows="8" />
          <!-- eslint-disable-next-line vue/no-v-html --> <!-- markdownPreview is user's own input sanitized by marked -->
        ...
        <small v-if="reportMarkdown.length > MAX_DISCORD_LENGTH" class="error-message">Details cannot exceed {{ MAX_DISCORD_LENGTH }} characters.</small>
      </div>
...
// Constants
const MAX_DISCORD_LENGTH = 2000 // even though this is not ref() we can somehow access this from the Vue template ...

// --- Remote Webhook Config ---
const WEBHOOK_EXT_REMOTE_URL = 'https://ssw.jku.at/General/Staff/Weninger/Projects/JavaWiz/bug_reporting.txt'
const webhookExt = ref<string | null>(null)
...
const isFormValid = computed(() => {
  return !!reportType.value &&
    ...
    reportMarkdown.value.length <= MAX_DISCORD_LENGTH &&
...
async function submitReport () {
  ...
  // Ensure webhook extension is loaded
  if (!webhookExt.value) {
    await loadWebhookExt()
  }
  const webhookUrl = buildWebhookUrl()

  const payload: any = await generateDiscordPayload();
  ...
  if (!response.ok) {
    ...
    const errorMessage = `Failed to send report to Discord. ${discordError}`
...
async function generateDiscordPayload () : Promise<any> {
  // 1. Upload Images
    ...
    embeds: [
      {
        ...
        description: `# ${reportType.value}: ${title.value}\n\n${reportMarkdown.value.substring(0, MAX_DISCORD_LENGTH)}`, // Ensure max length
        author: authorInfo,
        timestamp: timestampISO,
        color: reportTypeToColor()
      },
...
function reportTypeToColor () : number {
  switch (reportType.value) {
    ...
    case 'Feature Request':
      return 3447003 // Blue
    case 'Question':
      return 16448768 // Yellow
    default:
      return 0
  }
}

async function loadWebhookExt (): Promise<void> {
  const res = await fetch(WEBHOOK_EXT_REMOTE_URL, {cache: 'no-store'})
  if (!res.ok) {
    console.error('Failed to load webhook extension:', `HTTP ${res.status}`)
    webhookExt.value = null
    return
  }
  const raw = await res.text()
  // Helper to shift letter down by 1, wrapping around
  const shiftDown = (char: string): string => {
    if (/[a-z]/.test(char)) {
  ...
  // Process each line: trim, filter empty, shift characters, join lines
  webhookExt.value = raw
    .split(/\r?\n/)
    .map(l => l.trim())
    .filter(l => l.length > 0)
    .map(l => l.split('').map(shiftDown)
      .join(''))
    .join('')
    .replace(/^["']|["']$/g, '')
}
...
function buildWebhookUrl (): string {
  ...
  const ext = (webhookExt.value ?? '').trim()
  if (!ext) {
    throw new Error('Webhook configuration could not be loaded.')
  }
  // If the remote already provides a full URL, use it directly; otherwise, append to base
  return /^https?:\/\//i.test(ext) ? ext : (base + ext)
}
